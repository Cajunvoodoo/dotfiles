...
main' :: D.Client -> IO ()
main' dbus = do
  xmonad . docks . ewmh . ewmhFullscreen . {- dynamicSBs myStatusBarSpawner . -} dynProjects . keybindings . urgencyHook $ def
    { terminal           = myTerminal
...
clickable icon ws = addActions [ (ws, 1), ("q", 2), ("Left", 4), ("Right", 5) ] icon
                    -- where i = fromJust $ M.lookup ws myWS
...
myStatusBarSpawner (S s) = do
                    pure $ statusBarPropTo ("XMONAD_LOG_" ++ show s)
                          ("xmobar -x " ++ show s ++ " ~/dotfiles/home/programs/xmobar/xmobar" ++ show s ++ ".hs")
                          (pure $ myXmobarPP (S s))
...
myXmobarPP s  = filterOutWsPP [scratchpadWorkspaceTag] . marshallPP s $ def
  ...
  where
    ...
    layoutColorIsActive n l = do
      c <- withWindowSet $ return . W.screen . W.current
      if n == c then wrapL "<icon=/home/cajun/dotfiles/home/programs/xmobar/icons/" "_selected.xpm/>" l else wrapL "<icon=/home/amnesia/.config/xmonad/xmobar/icons/" ".xpm/>" l
